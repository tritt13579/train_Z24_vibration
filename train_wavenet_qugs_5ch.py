...
from qugs_trainval_5ch_dataset import QUGS5Cfg, make_loaders
...
# Output base dir (each run will create ./History_QUGS_5ch/v1, v2, ...)
HISTORY_DIR = "./History_QUGS_5ch"
...
def train_wavenet_qugs_5ch(
    ...
    """
    IMPORTANT:
    - Augmentation is handled INSIDE qugs_trainval_5ch_dataset.py (OFFLINE expanded index):
    ...
    ckpt_path = os.path.join(run_dir, f"wavenet_qugs_5ch_best_{version}.pt")
    ...
    with open(os.path.join(run_dir, f"wavenet_qugs_5ch_history_{version}.json"), "w", encoding="utf-8") as f:
        json.dump(history, f, ensure_ascii=False, indent=2)

    plot_history(history, os.path.join(run_dir, f"wavenet_qugs_5ch_curves_{version}.png"))
    plot_confusion_matrix(
        val_true,
        val_pred,
        num_classes,
        os.path.join(run_dir, f"wavenet_qugs_5ch_val_cm_{version}.png"),
        title=f"VAL Confusion Matrix (WaveNet QUGS 5ch) - {version}",
    )
