# QUGS train/val data processing for Structural Damage Detection
import os
import random
import re
...
class QUGSCfg:
    ...
    use_time_column: bool = False  # if False, drop time and keep only channels
    normalize: bool = True
    eps: float = 1e-8
...
def _load_qugs_file(fp: str, cfg: QUGSCfg, meta_cache: Dict) -> np.ndarray:
    if fp in meta_cache:
...
def _mean_std_from_train(index, cfg: QUGSCfg, meta_cache: Dict) -> Tuple[np.ndarray, np.ndarray]:
    sum_c = None
    sumsq_c = None
    count = 0
...
# DATASET
# =========================
class QUGSWindows(Dataset):
    def __init__(self, index, cfg: QUGSCfg, mean=None, std=None):
        self.index = index
        self.cfg = cfg
        self.mean = mean
        self.std = std
...
# BUILD LOADERS
# =========================
def make_loaders(cfg: QUGSCfg, batch_size=32, num_workers=0):
    ...
    train_ds = QUGSWindows(train_index, cfg, mean, std)
    val_ds = QUGSWindows(val_index, cfg, mean, std)
