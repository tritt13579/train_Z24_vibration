...
from qugs_trainval_dataset import QUGSCfg, make_loaders
...
# Dataset config
DATASET_ROOT = "./QUGS"
TRAIN_SUBDIR = "Dataset A"
VAL_SUBDIR = "Dataset B"

# Output base dir (each run will create ./History_QUGS/v1, v2, ...)
HISTORY_DIR = "./History_QUGS"
...
def train_wavenet_qugs(
    ...
    """
    IMPORTANT:
    - Augmentation is handled INSIDE qugs_trainval_dataset.py (OFFLINE expanded index):
    ...
    ckpt_path = os.path.join(run_dir, f"wavenet_qugs_full_best_{version}.pt")
    ...
    classes = [meta["classes"][i] for i in sorted(meta["classes"].keys())]
    num_classes = len(classes)
    input_channels = int(meta["mean"].shape[0]) if meta.get("mean") is not None else 30
    ...
    print(f"[DATA] train_samples={meta.get('train_samples')} val_samples={meta.get('val_samples')} "
    ...
    run_info = {
        ...
        "dataset": {
            ...
            "window": cfg.window,
            "step": cfg.step,
            "use_time_column": cfg.use_time_column,
            "augmentation": {
    ...
    with open(os.path.join(run_dir, f"wavenet_qugs_full_history_{version}.json"), "w", encoding="utf-8") as f:
        json.dump(history, f, ensure_ascii=False, indent=2)

    plot_history(history, os.path.join(run_dir, f"wavenet_qugs_full_curves_{version}.png"))
    plot_confusion_matrix(
        val_true,
        val_pred,
        num_classes,
        os.path.join(run_dir, f"wavenet_qugs_full_val_cm_{version}.png"),
        title=f"VAL Confusion Matrix (WaveNet QUGS full) - {version}",
    )
