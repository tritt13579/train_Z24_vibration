...
def _all_samples_for_class(files: List[str], y: int, cfg: Z24Cfg):
    starts = _starts(cfg)
    return [(fp, y, s) for fp in files for s in starts]


def _split_samples_70_20_10(samples: List[Tuple[str, int, int]], rng: random.Random):
    n = len(samples)
    if n < 3:
        raise ValueError(f"Need >=3 samples for 70/20/10, got n={n}")

    samples = samples[:]
    rng.shuffle(samples)

    n_train = int(round(0.70 * n))
    n_train = max(1, min(n_train, n - 2))  # leave at least 2
    remain = n - n_train

    n_val = int(round(0.20 * n))
    n_val = max(1, min(n_val, remain - 1))
    n_test = remain - n_val
    if n_test < 1:
        n_test = 1
        n_val = remain - 1
    ...
    tr = samples[:n_train]
    va = samples[n_train:n_train + n_val]
    te = samples[n_train + n_val:n_train + n_val + n_test]
    return tr, va, te
...
def make_loaders(classes: List[str], cfg: Z24Cfg, batch_size: int = 32, num_workers: int = 0):
    ...
    for y, cls in enumerate(classes):
        ...
        tr, va, te = _split_samples_70_20_10(all_samples, rng)
    ...
    te_ds = Z24Windows(te_idx, cfg, mean, std) if cfg.normalize else Z24Windows(te_idx, cfg)
