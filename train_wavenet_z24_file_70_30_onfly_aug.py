# train_wavenet_z24_file_70_30_onfly_aug.py
import os
...
def train_wavenet_file_split_70_30(
    ...
    """
    Augmentation is handled INSIDE dataset (on-the-fly, TRAIN only):
      - time shift roll
      - reverse (small prob)
      - amplitude scaling
      - gaussian noise with k ~ [0.03..0.07], biased small
    VAL uses clean only.
    """
    ...
    cfg = Z24Cfg(
        ...
        noise_k_min=0.03,
        noise_k_max=0.07,
        noise_bias_power=2.0,  # biased small
    )
    ...
    print("[AUG] ON-THE-FLY (TRAIN only): shift + reverse + amp_scale + noise(k biased small)")
    ...
    print(f"[AUG PARAMS] {meta.get('augment')}")
    ...
    run_info = {
        ...
        "dataset": {
            ...
            "window": cfg.window,
            "no_overlap": True,
            "normalize_train_stats_only": True,
            "augmentation": meta.get("augment"),
            "val_clean_only": True,
        }
    }
