...
class WaveNetClassifier(nn.Module):
    """
    Dataset should return x as [B, T, C].
    Internally: convert to [B, C, T] for Conv1D.
    """
    def __init__(
        self,
        num_classes: int,
...
def train_wavenet_file_split_70_30(
    ...
    """
    IMPORTANT:
    - Augmentation is handled inside z24_file_split_70_30.py (paper-style OFFLINE expansion):
      clean + noise(k=0.01*std) + reverse + crop(resample)
    - So DO NOT add extra Gaussian noise here.
    """
    ...
    # Dataset config: NOTE gap removed/ignored by the dataset file you updated
    cfg = Z24Cfg(
        root=DATASET_ROOT,
        subdir=DATASET_SUBDIR,
        key=DATASET_KEY,
        ...
        seed=SPLIT_SEED,
        normalize=True,
        return_ct=False,   # we want [T, C] from dataset for the model
        noise_k=0.01,
        crop_ratio_min=0.85,
        crop_ratio_max=0.95,
    )
    ...
    print("[AUG] Using dataset paper-style OFFLINE aug: clean + noise + reverse + crop(resample).")
